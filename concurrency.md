...
**Потокобезопасность (thread safety)** – свойство объекта или кода, которое гарантирует, что при исполнении или использовании несколькими потоками код будет вести себя корректно, как предполагается. Например, потокобезопасный счётчик не пропустит ни один счёт, даже если один и тот же экземпляр этого счётчика будет использоваться несколькими потоками.

**Уровни потокобезопасности (классификация Bloch, Effective Java):**

1. **Immutable (неизменяемый)** — объект нельзя изменить после создания. Всегда потокобезопасен без синхронизации. Примеры: `String`, `LocalDate`, Java Records.
2. **Unconditionally thread-safe** — объект мутабелен, но внутренняя синхронизация достаточна для безопасного использования из нескольких потоков. Примеры: `AtomicInteger`, `ConcurrentHashMap`.
3. **Conditionally thread-safe** — потокобезопасен для отдельных операций, но составные операции требуют внешней синхронизации. Пример: `Collections.synchronizedList()` — итерация требует внешнего `synchronized`.
4. **Not thread-safe** — требуется внешняя синхронизация для любого параллельного доступа. Примеры: `ArrayList`, `HashMap`.
5. **Thread-hostile** — небезопасен даже с внешней синхронизацией. Встречается крайне редко, обычно как результат ошибки.

**Основные механизмы обеспечения потокобезопасности в Java:**
...
С появлением **Virtual Threads** в Java 21 (Project Loom) концепция зелёных потоков вернулась в Java на новом уровне. Virtual Threads — это легковесные потоки, управляемые JVM, которые работают поверх пула platform threads (carrier threads). В отличие от классических green threads из Java 1.0-1.1, Virtual Threads:
- Могут использовать несколько ядер CPU (M:N модель — множество виртуальных потоков на нескольких ОС-потоках)
- Автоматически «отсоединяются» от carrier thread при блокирующих операциях (I/O, `sleep`, `Lock`)
- Полностью совместимы с существующим API (`Thread`, `ExecutorService`, `synchronized`)
- Позволяют создавать миллионы потоков с минимальными затратами памяти
...
Таким образом, в 2026 году Java фактически имеет три вида потоков: Platform Threads (обёртки над ОС-потоками), Virtual Threads (управляемые JVM) и, исторически, green threads (Java 1.0-1.1, устаревшие).
...
+ **(Java 21+)** Создать виртуальный поток через `Thread.ofVirtual().start(Runnable)` или использовать `Executors.newVirtualThreadPerTaskExecutor()`:

```java
// Platform Thread (традиционный)
Thread platformThread = Thread.ofPlatform().name("worker").start(() -> doWork());

// Virtual Thread (Java 21+)
Thread virtualThread = Thread.ofVirtual().name("vworker").start(() -> doWork());

// Executor с виртуальными потоками — один виртуальный поток на задачу
...
Формально в Java состояния потока определены в перечислении `Thread.State`: **NEW**, **RUNNABLE** (объединяет Ready и Running — JVM не разделяет их), **BLOCKED** (ждёт монитора `synchronized`), **WAITING** (бессрочное ожидание: `wait()`, `join()`, `LockSupport.park()`), **TIMED_WAITING** (ожидание с таймаутом: `sleep()`, `wait(ms)`, `join(ms)`), **TERMINATED** (завершён).

```java
Thread.State state = thread.getState(); // получить текущее состояние
```
...
new Thread(() -> {
    synchronized (lockA) {
        ...
        synchronized (lockB) { /* ... */ }  // ждёт lockB, который занят потоком 2
    }
}).start();
...
new Thread(() -> {
    synchronized (lockB) {
        Thread.sleep(100);
        synchronized (lockA) { /* ... */ }  // ждёт lockA, который занят потоком 1
    }
}).start();
// Результат: deadlock — оба потока ждут друг друга бесконечно
```

**Способы диагностики deadlock:**
...
- Использовать `tryLock(timeout)` вместо `synchronized` — позволяет сдаться при невозможности захватить блокировку
- Минимизировать области под блокировкой
- Использовать concurrent-коллекции и атомарные переменные вместо явных блокировок
...
+ **(Java 21+)** `newVirtualThreadPerTaskExecutor()` - создаёт новый виртуальный поток для каждой задачи. Не ограничен по количеству потоков. Рекомендуется для IO-bound задач. Реализует `AutoCloseable` — при закрытии ждёт завершения всех задач.
...
**Выбор пула потоков в 2026:**
- **IO-bound задачи** → `newVirtualThreadPerTaskExecutor()` (Java 21+) или `newCachedThreadPool()`
- **CPU-bound задачи** → `newFixedThreadPool(N)` где N ≈ количество ядер, или `ForkJoinPool`
...
С появлением Virtual Threads `ThreadLocal` создаёт серьёзные проблемы:
